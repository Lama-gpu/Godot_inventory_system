...
@onready var player = get_node_or_null("/root/worldScene/player")
...
func _on_slot_selected(slot: Slot):
	...
	info.text = selected_slot.get_nume() +" , "+ str(selected_slot.get_cantitate())
...
func drop_selected_item():
	if selected_slot:
		...
		if ID and is_instance_valid(player):
			var _world = get_node("/root/worldScene/")
...
func drop_item(ID: String, cantiti: int):
	...
	var item_scene = load("res://Scenes/Item.tscn") as PackedScene
	if item_scene and is_instance_valid(player) :
		var world_node = get_node("/root/worldScene/")
...
func drop_selected_item_1():
	if selected_slot:
		...
		if ID and is_instance_valid(player):
			...
			var world = get_node("/root/worldScene/")
